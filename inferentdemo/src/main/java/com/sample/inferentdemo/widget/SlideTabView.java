...
public class SlideTabView extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    //背景色
    private int background = R.color.colorDefault;
    //字体大小
    ...
    private void addTab() {
        for (int i = 0; i < list.size(); i++) {
            ...
            final LinearLayout view = (LinearLayout) View.inflate(context, R.layout.layout_play_time_title_item,null);
            TextView item = (TextView)view.findViewById(R.id.title_item);
    ...
    private void refresh(int index) {
        for (int i = 0; i < textViews.size(); i++) {
            if(i == index){
                textViews.get(i).setBackgroundColor(Color.rgb(0x75,0xc5,0xef));
                textViews.get(i).setTextColor(Color.rgb(0xff,0xff,0xff));
            }else{
                textViews.get(i).setTextColor(Color.rgb(0x99,0x99,0x99));
                textViews.get(i).setBackgroundColor(Color.rgb(0xff,0xff,0xff));
            }
        }
    }
